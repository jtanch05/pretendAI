...
export function DrawingCanvas({ value, onChange }: { value: DrawingData; onChange: (drawing: DrawingData) => void }) {
  ...
  return <section className="drawing-editor" aria-label={t("drawingEditor")}>
    <div className="drawing-toolbar" role="toolbar" aria-label={t("drawingTools")}>
      <span className="drawing-tool-label">{t("color")}</span>
      {colors.map((option) => <button key={option} className={color === option && !erasing ? "selected" : ""} style={{ backgroundColor: option }} type="button" aria-label={t("useColor", { color: option })} onClick={() => { setColor(option); setErasing(false); }} />)}
      <span className="drawing-tool-label">{t("brush")}</span>
      {sizes.map((option) => <button key={option} className={`brush-size ${size === option ? "selected" : ""}`} type="button" aria-label={t("brushSize", { size: option })} onClick={() => setSize(option)}><i style={{ width: option, height: option }} /></button>)}
      <button className={erasing ? "tool-button selected" : "tool-button"} type="button" onClick={() => setErasing((current) => !current)}>{t("eraser")}</button>
      <button className="tool-button" type="button" disabled={value.strokes.length === 0} onClick={() => onChange({ ...value, strokes: value.strokes.slice(0, -1) })}>{t("undo")}</button>
      <button className="tool-button" type="button" disabled={value.strokes.length === 0} onClick={() => onChange({ ...value, strokes: [] })}>{t("clear")}</button>
    </div>
    <svg className="drawing-surface" viewBox="0 0 640 400" role="application" aria-label={t("drawingCanvas")} onPointerDown={beginStroke} onPointerMove={continueStroke} onPointerUp={endStroke} onPointerCancel={endStroke}>
      <rect width="640" height="400" fill="#fff" />
    ...
    <p className="fine-print">{t("drawingHelp")}</p>
  </section>;
}
